...
public class BreakESP extends Module {
	...
	private final Setting<Color> color = this.register(new Setting<>("Color", new Color(255, 255, 255, 100)));
	...
	public void onRender3D(Render3DEvent event) {
		for (MineManager.BreakData breakData : new HashMap<>(OyVey.mineManager.breakMap).values()) {
			...
			Render3DUtil.draw3DBox(event.getMatrixStack(), new Box(breakData.pos).shrink(size, size, size).shrink(-size, -size, -size), color.getValue(), outline.getValue(), box.getValue());
