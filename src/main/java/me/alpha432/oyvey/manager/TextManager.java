...
public class TextManager extends Feature {
  ...
  /*  public void drawString(String text, float x, float y, int color, boolean shadow) {
        ...
        if (shadow) {
            Render2DEvent.drawWithShadow(matrices, text, x, y, color);
        } else {
...
/*
    public void updateResolution() {
        ...
        this.scaleFactor = mc.options.guiScale().get();
        if (this.scaleFactor == 0) {
            this.scaleFactor = 1000;
        }
        while (this.scaleFactor < mc.options.guiScale().get() && this.scaledWidth / (this.scaleFactor + 1) >= 320 && this.scaledHeight / (this.scaleFactor + 1) >= 240) {
            ++this.scaleFactor;
        }
        if (mc.options.guiScale().get() % 2 != 0 && this.scaleFactor != 1) {
            --this.scaleFactor;
        }
