...
import aids.dev.shrimphack.features.modules.client.hud.TimeDay;
import aids.dev.shrimphack.features.modules.client.hud.utils.FrameRateCounter;
...
public class HUD extends Module {
    ...
    private static final ItemStack totem = new ItemStack(Items.TOTEM_OF_UNDYING);
        ...
        private final Setting<String> customGreeter = this.register(new Setting<String>("GreeterCustom", "Shrimphack++", v -> this.greetermode.getValue() == Greeter.CUSTOM));
    ...
    public HUD() {
        super("HudNew", "hud", Category.CLIENT, true, false, false);
    }
    ...
    public void onRender2D(Render2DEvent event) {
        ...
        if (arrayList.getValue())
            if (renderingUp.getValue()) {
                for (Module module : Shrimphack.moduleManager.getEnabledModules().stream().filter(Module::isDrawn).sorted(Comparator.comparing(module -> mc.textRenderer.getWidth(module.getFullArrayString()) * -1)).collect(Collectors.toList())) {
                    if (!module.isDrawn()) {
                        continue;
                    }
        ...
        if (renderingUp.getValue()) {
            if (speed.getValue()) {
                String str = "Speed " + Formatting.WHITE + MathUtility.round((float) (Shrimphack.hudManager.currentPlayerSpeed * 72f)) + " km/h";
                i += 10;
            ...
            if (tps.getValue()) {
                String str = "TPS " + Formatting.WHITE + Shrimphack.hudManager.getTPS() + (extraTps.getValue() ? " [" + Shrimphack.hudManager.getTPS2() + "]" : "");
                i += 10;
                event.getContext().drawTextWithShadow(mc.textRenderer, Text.of(str), (width - getStringWidth(str) - 2), (height - 2 - i), color);
            }
            String fpsText = "FPS " + Formatting.WHITE + FrameRateCounter.INSTANCE.getFps();
            String str1 = "Ping " + Formatting.WHITE + Shrimphack.hudManager.getPing();
        ...
        int widthGreeter = mc.getWindow().getScaledWidth();
            ...
            switch (this.greetermode.getValue()) {
                case CHRISTMAS: {
                    text = "Shrimphack++ " + "time remove" + HUD.mc.player.getDisplayName().getString() + " :^)";
                    break;
                }
            }
            switch (this.greetermode.getValue()) {
                case LONG: {
                    text = "Welcome to Shrimphack++ " + HUD.mc.player.getDisplayName().getString() + " :^)";
                    break;
                }
            }
            switch (this.greetermode.getValue()) {
