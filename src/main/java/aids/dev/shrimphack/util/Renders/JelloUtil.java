...
public class JelloUtil implements Util {
    ...
    public static void drawJello(MatrixStack matrix, Entity target, Color color) {
        ...
        for (int i = 0; i <= 30; i++) {
            ...
            bufferBuilder.vertex(matrix.peek().getPositionMatrix(), cos, (float) y, sin).color(ColorUtils.injectAlpha(color, 0).getRGB()).next();
        }
