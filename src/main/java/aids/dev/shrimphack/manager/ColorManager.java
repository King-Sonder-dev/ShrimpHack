...
import aids.dev.shrimphack.features.gui.Component;
...
public class ColorManager {
    ...
    public int getColorAsInt() {
        return ColorUtils.toRGBA(this.color);
    }
    ...
    public int getColorAsIntFullAlpha() {
        return ColorUtils.toRGBA(new Color(this.color.getRed(), this.color.getGreen(), this.color.getBlue(), 255));
    }
    ...
    public int getColorWithAlpha(int alpha) {
        if (ClickGui.getInstance().rainbow.getValue()) {
            return ColorUtils.rainbow(Component.counter1[0] * ClickGui.getInstance().rainbowHue.getValue()).getRGB();
        }
        return ColorUtils.toRGBA(new Color(this.red, this.green, this.blue, (float) alpha / 255.0f));
    }
